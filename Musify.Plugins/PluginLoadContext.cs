...
public class PluginLoadContext : AssemblyLoadContext
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Manifest manifest = await Manifest.FromPluginArchivetAsync(pluginArchive, cancellationToken)?? throw new PluginNotLoadedException(path, null, new("Plugin archive does not contain a manifest or is badly formatted."));
        PluginLoadContext loadContext = new(manifest);

        loadContext.EntryPointAssembly = await loadContext.LoadFromArchiveAsync(pluginArchive, manifest.EntryPoint, cancellationToken) ?? throw new PluginNotLoadedException(path, manifest, new($"Entry point assembly could not be loaded: [{manifest.EntryPoint}]."));
        foreach (string dependency in manifest.Dependencies)
            if (await loadContext.LoadFromArchiveAsync(pluginArchive, dependency, cancellationToken) is null)
                throw new PluginNotLoadedException(path, manifest, new($"Dependency assembly could not be loaded: [{dependency}]."));
